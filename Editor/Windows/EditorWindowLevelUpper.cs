...
namespace LevelUpper.Editor {
	...
	/// <summary> Base class for EditorWindows that provides a lot of convinient GUILayout stuff. </summary>
	public class EditorWindowLevelUpper : EditorWindow {

		/// <summary> Has something in this EditorWindow changed? Built-in controls automatically detect changes if <see cref="checkChanges"/> is set. </summary>
		[NonSerialized] public bool changed = false;
		/// <summary> Should <see cref="changed"/> be updated by built-in controls? </summary>
		[NonSerialized] public bool checkChanges = false;
		/// <summary> Color to use to notifiy a change has happenend. </summary>
		public virtual Color changedColor { get { return new Color(1, .5f, .5f, 1); } }

		/// <summary> Shortcut to get the width of this window. </summary>
		...
		/// <summary> Set the <see cref="GUI.color"/> to <see cref="changedColor"/> if something has been <see cref="changed"/>. Uses <see cref="Color.white"/> as the default color. </summary>
		public void SetChangedColor() { SetChangedColor(Color.white, changed); }
		/// <summary> Set the <see cref="GUI.color"/> to <see cref="changedColor"/> if something has been <see cref="changed"/>. Uses the given <paramref name="defaultColor"/>. </summary>
		/// <param name="defaultColor"> Default color to use. </param>
		public void SetChangedColor(Color defaultColor) { SetChangedColor(defaultColor, changed); }
		/// <summary> Set the <see cref="GUI.color"/> to <see cref="changedColor"/> if something has been <paramref name="changed"/>. Uses <see cref="Color.white"/> as the default color. </summary>
		/// <param name="changed"> If the current state has been changed from some previous saved state. </param>
		public void SetChangedColor(bool changed) { SetChangedColor(Color.white, changed); }
		/// <summary> Set the <see cref="GUI.color"/> to <see cref="changedColor"/> if something has been <paramref name="changed"/>. Uses <see cref="Color.white"/> as the default color. </summary>
		/// <param name="changed"> If the current state has been changed from some previous saved state. </param>
		/// <param name="defaultColor"> Color to use if <paramref name="changed"/> is false. </param>
		public void SetChangedColor(Color defaultColor, bool changed) { GUI.color = changed ? changedColor : defaultColor; }
		...
		public bool KeyDown(KeyCode code) { return hasFocus && Event.current.type == EventType.KeyDown && Event.current.keyCode == code; }
		...
		public bool KeyUp(KeyCode code) { return hasFocus && Event.current.type == EventType.KeyUp && Event.current.keyCode == code; }

		/// <summary> Check the current event to see if it is a MouseDown of the given mouse <paramref name="button"/> </summary>
		/// <param name="button"></param>
		/// <returns></returns>
		public bool MouseDown(int button) { return hasFocus && Event.current.type == EventType.MouseDown && Event.current.button == button; }

		/// <summary> Check the current event to see if it is a MouseUp of the given mouse <paramref name="button"/> </summary>
		/// <param name="button"></param>
		/// <returns></returns>
		public bool MouseUp(int button) { return hasFocus && Event.current.type == EventType.MouseUp && Event.current.button == button; }
